...
type AuctionPageClientProps = {
    vehicleData: VehicleModel;
}



const AuctionPageClient = ({ vehicleData }: AuctionPageClientProps) => {
                                ...
                                <div className="md:col-span-8">
                                    {vehicleData?.display_type != "auction" ? (
                                        ...
                                        <div className='flex items-start'>
                                            {vehicleData?.auction?.top_bid === null ?
                                                <>
                                                    <h1 className="font-semibold text-xl text-orange-400">KES {parseInt(vehicleData?.auction?.starting_price).toLocaleString()}</h1>
                                                </>
                                                :
                                                <>
                                                    <h1 className="font-semibold text-xl text-orange-400">KES {parseInt(vehicleData?.auction?.top_bid?.amount).toLocaleString()}</h1>
                                                </>
                                            }
                                            <p className='text-slate-500 text-xs'>Top Bid</p>
                                        </div>
                                    )}
                            ...
                            {vehicleData?.display_type === "auction" && (
                                <div className='bg-orange-50 border border-orange-400 p-8 my-5 rounded-2xl'>
                                    <div className='grid md:grid-cols-2 grid-cols-1 gap-8'>
                                        <div className=''>
                                            {vehicleData?.auction?.status === "upcoming" && (
                                                <div className='py-1'>Auction starts in: <CountDown endTime={vehicleData?.auction?.start_time} /></div>
                                            )}
                                            {vehicleData?.auction?.status === "live" && (
                                                <div className='py-1'>Auction ends in: <CountDown endTime={vehicleData?.auction?.end_time} /></div>
                                            )}
                                            {vehicleData?.auction?.status === "ended" && "Auction ended"}
                                        </div>
                                        <div>
                                            <div>
                                                <p className='text-slate-500'>Top Bid:</p>
                                                {vehicleData?.auction?.top_bid === null ?
                                                    <>
                                                        <h1 className="font-semibold text-xl">KES {parseInt(vehicleData?.auction?.starting_price).toLocaleString()}</h1>
                                                    </>
                                                    :
                                                    <>
                                                        <h1 className="font-semibold text-xl">KES {parseInt(vehicleData?.auction?.top_bid?.amount).toLocaleString()}</h1>
                                                    </>
                                                }
                                            </div>
                                        </div>
                        ...
                        {vehicleData?.display_type != "auction" ? (
                            ...
                            <Card className='mb-8'>
                                <CardContent>
                                    ...
                                    <div>
                                        {vehicleData?.auction?.status === "upcoming" && (
                                            <div className='py-5'>Auction starts in: <CountDown endTime={vehicleData?.auction?.start_time} /></div>
                                        )}
                                        {vehicleData?.auction?.status === "live" && (
                                            <div className='py-5'>Auction ends in: <CountDown endTime={vehicleData?.auction?.end_time} /></div>
                                        )}
                                        {vehicleData?.auction?.status === "ended" && "Auction ended"}


                                    </div>
                                    <div><p className='text-slate-500'>Top Bid:</p>
                                        {vehicleData?.auction?.top_bid === null ? 
                                            <>
                                                <h1 className="font-semibold text-xl">KES {parseInt(vehicleData?.auction?.starting_price).toLocaleString()}</h1>
                                            </> 
