...
interface BusinessWebsiteFormProps {
    initialData: string;
    business_id: string;
}
...
const BusinessWebsiteForm = ({ initialData, business_id }: BusinessWebsiteFormProps) => {
    ...
    useEffect(() => {
        if (initialData) {
            form.reset({ website: initialData });
        }
    }, [initialData]);
        ...
        <section className="bg-white border-2 border-slate-100 rounded-lg p-4">
            <div className="flex flex-col">
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex text-sm text-slate-600">
                        <Button variant="ghost" className="cursor-pointer" onClick={toggleEdit}>
                            ...
                            {!isEditing && (
                                <>
                                    Edit website <PencilIcon className="w-4 h-4" />
                                </>
                            )}
                ...
                {isEditing ? <>
                                    ...
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input
                                                type="url"
                                                className="bg-white"
                                                placeholder="e.g. https://kenautos.co.ke"
                                                {...field}
                                            />
                                        </FormControl>
                                        <FormMessage />
                    ...
                    <p className="text-slate-500 text-sm mt-2">{initialData ? initialData : "No website link"}</p>
                </>}
...
export default BusinessWebsiteForm