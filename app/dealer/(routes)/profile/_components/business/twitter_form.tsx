...
const TwitterForm = ({ initialData, business_id }: TwitterFormProps) => {
    ...
    useEffect(() => {
        if (initialData) {
            form.reset({ twitter: initialData });
        }
    }, [initialData, form]);
        ...
        <section className="bg-white border-2 border-slate-100 rounded-lg p-4">
            <div className="flex flex-col">
                ...
                {isEditing ? <>
                    ...
                    <p className="text-slate-500 text-sm mt-2">{initialData ? initialData : "No X link"}</p>
                </>}
